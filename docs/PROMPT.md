...
- гарантировать, что `AntiGoblin` затрагивает только политику `xkeen` и не ломает другие политики пользователя.
...
- `xkeen` должна существовать как отдельная UI-видимая политика Keenetic;
- bootstrap обязан создавать ее автоматически;
- пользователь не должен создавать `xkeen` руками;
- существующие пользовательские политики вроде `no_vpn` трогать нельзя.
- если `xkeen` удалили, self-heal должен уметь создать ее заново без зацепа чужих политик.
- минимальный подтвержденный путь восстановления `xkeen`:
  - создать `Policy42+`
  - задать `description xkeen`
  - задать `permit global <WAN>`
  - выполнить `system configuration save`
- привязка устройств к policy не является обязательной частью восстановления самой `xkeen`.
- для автоматического восстановления опирайся прежде всего на watchdog `S25antigoblin-selfheal`, а не только на cron.
- не считай `порт 61219 открыт` достаточной проверкой здоровья `xray`;
- при разборе деградации смотри отдельный health-log:
  - `/opt/var/log/xkeen-health.log`
- в нормальном режиме жди в нем health-snapshot примерно раз в 5 минут
- в health-проверках учитывай:
  - наличие `xray` pid
