...
Это набор рабочих заметок и скриптов по маршрутизации VPN на Keenetic.
...
- `HydraRoute` оставлен как selector/UI
- `XKeen` поднят как рабочий transport path
...
Во время миграции/отладки было сделано:
...
- применены пользовательские drafts для:
  - outbounds
  - routing
- исправлен generated `S24xray`
- убран несовместимый global `transport` template
